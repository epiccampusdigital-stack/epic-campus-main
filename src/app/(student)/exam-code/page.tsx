...
export default function ExamCodePage() {
  ...
  async function handleSubmit() {
    if (code.length !== 5) { setError('Please enter the 5-digit exam code'); return }
    setLoading(true)
    setError('')
    try {
      ...
      if (data.codeExpiresAt && data.codeExpiresAt < now) {
        setError('Code has expired — ask teacher for new code')
        setLoading(false)
        return
      }
    ...
    <div className="flex min-h-[60vh] items-center justify-center p-4">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-[#5A6A7A] dark:text-white/50">Ask your teacher for today&apos;s 5-digit code</p>
        </div>
        ...
        <div className="rounded-2xl border border-[#DDE3EC] dark:border-white/[0.08] bg-white dark:bg-white/[0.04] p-6 space-y-4">
          <div>
            ...
            <input
              ...
              value={code}
              onChange={e => {
                // Numbers only — strip any non-digit and cap at 5 characters.
              ...
              className="w-full rounded-xl border-2 border-[#DDE3EC] dark:border-white/20 bg-[#F5F7FB] dark:bg-white/[0.04] px-4 py-4 text-center font-mono text-4xl font-black text-[#0B3D6B] dark:text-white outline-none focus:border-[#E8A020] tracking-widest"
            />
          </div>
